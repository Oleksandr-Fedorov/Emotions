...
const LEVEL_STEPS = [
  { target: 'neutral', label: 'Сделай серьезное лицо 😐', allowed: ['neutral'] },
  { target: 'positive', label: 'Улыбнись широко! 😃', allowed: ['positive'] },
  { target: 'negative', label: 'Покажи злость! 😡', allowed: ['negative'] }
];
          ...
          {isIntro && (
            <div className="intro-overlay">
              <div className="intro-content">
                ...
                <p className="intro-text">Нужно выполнить 3 эмоции.<br/>Как только покажешь верно — сразу следующий шаг.</p>
                <button className="start-button" onClick={handleStart}>Погнали</button>
              </div>
            </div>
          )}
            ...
            <div className="dialogue-section">
              ...
              <div className="chat-bubble">
                <div style={{display: 'flex', flexDirection: 'column'}}>
                    <h3>{isTransitioning ? "Супер! 👍" : LEVEL_STEPS[stepIndex].label}</h3>
                    {!isTransitioning && (
          ...
          {isLevelComplete && (
            <div className="intro-overlay">
              <div className="intro-content">
                ...
                <button className="start-button" onClick={onNextLevel}>Дальше →</button>
              </div>
            </div>
          )}
        </div>
